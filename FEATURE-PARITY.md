...
- [x] **Nurture sequences / drip campaigns** — Recruitment `nurture_sequences` / `nurture_steps` / `nurture_enrollments` (migration `NurtureSequences`): create a sequence, append ordered steps (per-step delay-days + subject/body), enrol recipients (dedup per sequence), and a `NurtureScheduler` background service (default on, `Nurture:IntervalSeconds`) that advances due enrolments — sending each step via the existing `CampaignEmailRequested` outbox event → Notifications worker, then scheduling the next step or completing. Endpoints `GET/POST /nurture`, `/nurture/{id}`, `/nurture/{id}/steps`, `/nurture/{id}/enroll`, `/nurture/enrollments/{id}/stop` (admin/-write); admin "Nurture sequences" panel (build steps, enrol, view/stop enrolments). 5 unit tests incl. the runner (Recruitment 158); integration suite green (migration applies on startup). **v0.3.0 #109 shipped.** *(Stop-on-reply needs inbound-email; manual stop ships now.)*
- [x] **Self-schedule interview booking** — recruiter offers proposed slots (`InterviewBookingSlot`: future-time/duration/location validation) for an application; the candidate books one from their portal → it **schedules the real `Interview`** at that time and **expires the sibling slots**. `interview_booking_slots` table (migration `InterviewKitsAndBooking`); `GET/POST /applications/{id}/booking-slots` (offer admin-write, read auth) + `POST /booking-slots/{id}/book` (auth). Admin "Self-schedule slots" in the interview drawer + talent "Pick an interview time" in the shared applications panel. Real-repo integration test (offer→book→interview + sibling-expire + re-book conflict). *(A full availability-calendar engine is a later step; recruiter-proposed slots ship now.)*
- [x] **Reusable interview kits / question banks** — `interview_kits` + `interview_kit_questions` (each question tags the skill it assesses, mapping onto the per-round `interview_skill_ratings`); `GET/POST /interview-kits`, `/interview-kits/{id}`, `/interview-kits/{id}/questions` (admin/-write); admin "Interview kits" panel. 7 unit tests across both (Recruitment 165). **v0.3.0 #110 shipped** — Workstream B pure-engineering complete.
- [x] **Skills taxonomy / ontology v1** — pure `SkillTaxonomy` in `Illumin360.Matching`: a curated alias→canonical catalog (`Canonicalize`/`CanonicalId`/`Dedupe`/`DuplicateGroups`); unknown skills pass through cleaned (nothing lost). `SkillGapAnalyzer` gains an opt-in `useTaxonomy` mode (default off keeps old behaviour) so synonyms match ("JS" ⇔ "JavaScript"); Professionals `/me/skill-gap` now uses it, plus `GET /me/skills/canonical` maps a profile onto canonical names + flags duplicate/synonym skills to merge. Professional-portal "Tidy up skills" surface. Dependency-free (inference from work history is the later step); 15 unit tests (Matching 49, Professionals 25). **v0.3.0 #111 shipped.**
- [~] **Real embedding model + pgvector** `[DECISION: data egress]` — **abstraction shipped, defaulted OFF** (#112): async `IEmbeddingClient` port + `HostedEmbeddingClient` (OpenAI-compatible `/embeddings`) + `EmbeddingOptions` in `Illumin360.Matching`, selected by config (`Matching:Embeddings` — hashing default; hosted requires `Enabled=true` + an endpoint, the egress gate). `HashingEmbeddingProvider` now implements the async port too; `SemanticRanker.RankAsync` + the Candidates semantic-similar handler use it. 4 new unit tests (Matching 58). *Remaining: flip the hosted flag on (needs the DPA/egress sign-off + endpoint/key) and add pgvector persistence — deferred until a hosted model is actually enabled.*
- [x] **GenAI assistant** `[DECISION: LLM egress — flag off by default]` — new `Illumin360.Ai` building block: `ITextCompletionClient` port + `HostedTextCompletionClient` (OpenAI-compatible `/chat/completions`) + `AiOptions` (default `Disabled`; `Hosted` needs `Enabled=true` + an `Endpoint`, the egress gate) + pure `AiTemplates` deterministic fallbacks. Recruitment `POST /ai/{job-description,summarize,draft-message}` (AdminPolicy) each **use the hosted model when enabled, else a local template** — so the features always work offline and upgrade to real GenAI when opted in; the response carries `source: hosted|template`. Admin "AI writing assistant" panel (3 tools + source badge). Config registers the backend in the Api (disabled default). 5 new unit tests (Recruitment 175). *(Conversational chatbot + autonomous agents remain Tier 2.)*
