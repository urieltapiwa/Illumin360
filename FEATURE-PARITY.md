...
- [ ] **Semantic / embedding matching** — encode résumé + JD to vectors (Sentence-BERT / pgvector-style), score by cosine similarity to catch implicit/paraphrased skills keyword-matching misses. **Design/decision doc:** [`03-architecture/semantic-matching-design.md`](03-architecture/semantic-matching-design.md) — proposes pgvector + a pluggable `IEmbeddingProvider` (deterministic hashing default, hosted API opt-in); **open decisions (provider, data-egress sign-off, v1 scope) block implementation**
