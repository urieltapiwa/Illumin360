...
- [~] **Feedback-loop learning** — feed recruiter accept/reject + hire outcomes back as ranking signal (learning-to-rank). **Data capture shipped + feature-enriched:** every terminal decision records a labelled `match_outcomes` row (deduped 1:1) with the ranker match-score **plus a Recruitment-owned feature snapshot — arrival source, role remote flag, interview count, avg interview rating, offer-made, days-to-decision**; `GET /metrics/outcomes` summary + `GET /metrics/outcomes/export.csv` (features-first / label-last, LTR-ready) + an admin "Hiring outcomes" panel (counts, avg score by outcome, CSV export). *Training a re-ranker on the accumulated labelled feature rows is the remaining step — deferred until enough decisions exist; cross-service talent-side features (skills/city) would need capture at apply-time, a further follow-up.*
