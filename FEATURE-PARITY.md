...
- [~] **Feedback-loop learning** — feed recruiter accept/reject + hire outcomes back as ranking signal (learning-to-rank). **Data capture shipped:** every terminal decision now records a labelled `match_outcomes` row (application, request, talent, talent-type, ranker match-score, hired/rejected, decided-at) in Recruitment, deduped 1:1; `GET /v1/recruitment/metrics/outcomes` + an admin "Hiring outcomes" panel show the growing training set and whether the current ranker separates hires from rejections (avg score by outcome). *Training a re-ranker on the accumulated labels is the remaining step — deferred until enough labelled decisions exist.*
