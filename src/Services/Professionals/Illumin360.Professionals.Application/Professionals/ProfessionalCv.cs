...
/// <summary>Metadata describing the current ("me") professional's uploaded CV.</summary>
...
/// <summary>Uploads (or replaces) the current ("me") professional's CV.</summary>
/// <param name="FileName">Original file name.</param>
/// <param name="ContentType">MIME type.</param>
...
/// <summary>Reads the current ("me") professional's CV metadata.</summary>
public sealed record GetCvMetadataQuery : IQuery<CvDto>;

/// <summary>Downloads the current ("me") professional's CV content.</summary>
...
public sealed class UploadCvCommandHandler(IProfessionalRepository repository, IObjectStorage storage)
    : ICommandHandler<UploadCvCommand, CvDto>
{
    ...
    public async Task<Result<CvDto>> HandleAsync(UploadCvCommand command, CancellationToken cancellationToken)
    {
        ...
        var key = $"professionals/{id.Value}/cv{ext}";
...
/// <param name="repository">The professional repository.</param>
public sealed class GetCvMetadataQueryHandler(IProfessionalRepository repository)
    : IQueryHandler<GetCvMetadataQuery, CvDto>
{
    ...
    /// <summary>Resolves the default ("me") professional (tracked), or null if none exist.</summary>
    /// <param name="repository">The professional repository.</param>
    /// <param name="ct">Cancellation token.</param>
    /// <returns>The professional, or null.</returns>
    internal static async Task<Domain.Professional?> ResolveMeAsync(IProfessionalRepository repository, CancellationToken ct)
    {
        var meId = await repository.GetDefaultProfessionalIdAsync(ct).ConfigureAwait(false);
...
/// <param name="repository">The professional repository.</param>
/// <param name="storage">Object storage.</param>
public sealed class DownloadCvQueryHandler(IProfessionalRepository repository, IObjectStorage storage)
